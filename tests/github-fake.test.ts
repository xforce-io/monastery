...
test("reactions: defaults to [], returns injected reaction contents (with author) for a comment", async () => {
  const gh = new FakeGitHub({ thesis: "T", issues: [] });
  ...
  gh.reactionAuthor["42"] = "alice";
  expect((await gh.reactions("o/r", "42"))[0]).toEqual({ content: "+1", author: "alice" });
});
