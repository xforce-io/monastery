...
function buildContext(input: MaintainerInput): string {
  ...
  return [
    ...
    [
      ...
      `- {"kind":"propose","num":${issue.number},"proposal":"close"|"merge"|"decline","draft":"<markdown the human will see>"} — ask a human to approve a risky, irreversible action. Use "decline" when the right PM call is to stop processing this issue as not worth doing / out of scope / no longer wanted; ONLY after a real human 👍 does the shell mark it monastery:declined.`,
